...
# networks/model_reg.py
import torch

# --- Minimal model registry used by mobilemamba.py ---
class _Registry:
    def __init__(self): self._mods = {}
    def register_module(self, fn):
        self._mods[fn.__name__] = fn
        return fn
    def get(self, name): return self._mods[name]
    def __contains__(self, name): return name in self._mods

MODEL = _Registry()

# --- Robust checkpoint loader (no timm dependency) ---
def load_checkpoint(model, ckpt_path, strict=True, map_location="cpu"):
    ...
    if missing:
        print(f"[load_checkpoint] Missing keys ({len(missing)}): {missing[:10]}{' ...' if len(missing)>10 else ''}")
    if unexpected:
        print(f"[load_checkpoint] Unexpected keys ({len(unexpected)}): {unexpected[:10]}{' ...' if len(unexpected)>10 else ''}")
    if strict and (missing or unexpected):
        raise RuntimeError("Strict load failed due to missing/unexpected keys.")
    return model
