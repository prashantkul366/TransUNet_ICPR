...
class MambaBlock(nn.Module):
    def __init__(self, config, vis):
        ...
        self.core = TransformerMambaBlock(
            dim=config.hidden_size,
            num_heads=config.transformer["num_heads"],
            mlp_ratio=config.transformer.get("mlp_ratio", 4.0),
            d_state=config.transformer.get("d_state", 8),
            d_conv=config.transformer.get("d_conv", 3),
            expand=config.transformer.get("expand", 1),
        )

    def forward(self, x):
        # x: (B, N, hidden_size)
        x = self.core(x)
        weights = None   # if you don’t need attention maps
        return x, weights

    # Optional: stub to keep load_from() calls from crashing
    def load_from(self, weights, n_block):
        # Either leave empty or implement custom loading for your block
        pass
